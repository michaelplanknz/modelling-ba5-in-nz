...
% Code edited by Michael Plank, June/July 2022 - as described in comments
...
if isa(data, 'dataset') || isstruct(data) || istable(data)
    ...
    if isempty(grouporder)
       ...
       % MJP edit: commented out to
       % maintain order of Table columns rather than sorting
       % alphabetically:
    ...
    for n=1:length(catnames)
        ...
        % MJP EDIT 3/7/22 
        % This was causing Violin to throw an error with table data is
        % input because this "thisData" is a numerical array and Violin is
        % expecting a cell variable containing the numerical array. So
        % convert to cell variable with {.} (as done below for numeric
        % input with the line "data = {data}"
        thisData = {thisData};
        % EDIT ENDS
